          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {materials.map((m, i) => (
              <div key={i} className="rounded-2xl overflow-hidden shadow-lg group cursor-pointer relative">
                ...
                {m.type === "download" && (
                  <div className="absolute inset-0 bg-primary/60 flex flex-col items-center justify-center text-card">
                    <Download className="w-10 h-10 mb-2" />
                    <p className="font-display text-xl font-bold">{m.title}</p>
                    {m.subtitle && <p className="font-body text-sm opacity-80 mt-1">{m.subtitle}</p>}
                  </div>
                )}
              </div>
            ))}
