    ...
    <Layout>
      <section className="py-16 md:py-24 px-4 relative">
        ...
        <div className="container mx-auto max-w-4xl relative z-10">
          <h1 className="section-title mb-6 uppercase underline underline-offset-4 text-secondary">
            Transparência
          </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6">
            {cards.slice(0, 3).map((card, i) => (
              ...
              >
                <span className="font-display text-xl font-bold text-primary-foreground whitespace-pre-line leading-tight">{card.label}</span>
                <div className="w-10 h-10 rounded-full border-2 border-white flex items-center justify-center mt-4 shrink-0">
                  {card.isDownload ? <Download className="w-5 h-5 text-primary-foreground" /> : <Plus className="w-5 h-5 text-primary-foreground" />}
                </div>
              </Link>
            ))}
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-lg mx-auto">
            {cards.slice(3).map((card, i) => (
              ...
              >
                <span className="font-display text-xl font-bold text-center whitespace-pre-line text-primary-foreground">{card.label}</span>
